...
@author: uri
First, generate all possible trees. The set is number_of_features x4 = 88 trees in our case.
Next, algorithm iterated over all trees and adjusted tree alpha. At the end of each iteration the algorithm checked if loss function is still shrinking. 
The algorithm stops when loss function result grew as compared to previous iteration. 
'''
import csv
import math
...
class OneLevelTree(object):
    def __init__(self,train,column,clf_true=None,clf_false=None):
        self.train = train
        self.head=d_node(column,'1',None,None,None)
        self.column=column
        if clf_true is not None:
            self.head.true_node = d_node(self.column,'1',None,None,clf_true)
            self.head.false_node = d_node(self.column,'1',None,None,clf_false)
...
class CoordinateDescent(object):
    def __init__(self,train,column_order=None):
        ...
        if column_order==None:
            init_alpha=1/(4*(len(train[0])-1))
            for column in range(1,len(train[0])):
                tree=OneLevelTree(train,column,clf1,clf1)
                self.trees.append(tree)
                tree=OneLevelTree(train,column,clf1,clf2)
                self.trees.append(tree)
                tree=OneLevelTree(train,column,clf2,clf1)
                self.trees.append(tree)
                tree=OneLevelTree(train,column,clf2,clf2)
                self.trees.append(tree)
