...
class decision_tree(object):
    ...
    def fit_model_greedy(self):
        ...
        for col in range(1,columns_num):
            value = '0'
            set1,set2=self.split_data_set(self.train_data, col, value)
            t_node =  d_node(classification=self.get_uniques(set1))
            f_node = d_node(classification=self.get_uniques(set2))
            head_node = d_node(column=col,value='0',true_node=t_node,false_node=f_node)
...
class AdaBoost(object):
    ...
    def update_weights(self,weights,alpha,error,results,labels):
        ...
        for i,w in enumerate(weights):
            #print(labels[i],results[i],i,w,(math.exp((-1)*labels[i]*results[i])*alpha))
            #print(2*math.sqrt(error*(1-error)))
            #if i==0:
                #print(math.exp((-1)*labels[i]*results[i]*new_alpha))
                #print((math.exp((-1)*labels[i]*results[i]*new_alpha))/(2*math.sqrt(error*(1-error))))
...
if __name__ == '__main__':
    ...
    model=AdaBoost(train,M=16,tree_depth=0)
    model.fit_greedy()
    for i in range(16):
